...
while not quit:
    ...
    if save:
        if done and os.path.exists(demo_file_name):
            os.remove(demo_file_name)
        elif not done:
            env.save_to_file(demo_file_name)
        
        save_as_pickled_object(demonstrations, args.game + time.strftime("%Y-%m-%d-%H-%M-%S", time.localtime())+".bin")
        quit = True
    ...
    for step in range(args.frame_skip):
        ...
        #print("Size of observation:", np.asarray(observation).shape)
        demonstration = {'observation': observation, 'reward': reward, 'done': done, 'info': info, 'action': action}
        if ACTION_KEYS['TIMETRAVEL'].issubset(key_presses):
            demonstrations.pop()
        elif done:
            pass
        else:
            demonstrations.append(demonstration)
        print("Size of demonstrations:", np.asarray(demonstrations).shape)
